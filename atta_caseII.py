...
# state: defines shared state between this thread and parent thread
# state['decider_lock']: defines lock
def decider_thread(state: dict):
    print("Decider thread started.")
    ...
    iter = 0
    load_file_name = "TA_normaldist_tasks_" + str(iter) + ".mat"
    ...
    while not state['shutdown']:
        ...
        # NOTE:
        # (1) Wait for signal from Allocator before starting
        state['decider_lock'].acquire()
        while state['decider_pending'] or state['shutdown']:
            if state['shutdown']:
                print("Decider thread finished.")
                state['decider_lock'].release()
                return
            state['decider_cv'].wait()

        # (1a) Copy task data to shared state
        state['task'] = copy.deepcopy(p[:,i])
        ...
        # NOTE:
        # (2) Update shared state with decision
        state['decision'] = assigned

        # NOTE:
        # (3) Release lock to allow Allocator to finish startTask()
        state['decider_pending'] = True
        state['decider_cv'].notify_all()
        state['decider_lock'].release()


        # NOTE:
        # (4) Wait for Allocator to receive the task's outcome
        state['decider_lock'].acquire()
        while state['decider_pending'] or state['shutdown']:
            if state['shutdown']:
                print("Decider thread finished.")
        ...
        # NOTE:
        # (4a) Obtain outcome from shared state
        ...
        obs_probs = np.divide(
            total_successes, 
            total_obs, 
            out=np.zeros_like(total_successes),
            where=total_obs!=0
        ) #divide the # of successes in each cell by the # of observations
            ...
            if ll.item() < loss_tolerance: #.item is just the way to extract the value from a torch tensor
                ...
                while t < 2200: #I chose 2200. 1520 was a good number of iterations to converge on the true l1,u1,l2,u2 values for Hebert's simulation, otherwise there are just oscillations around the actual capability star value
                    ...
                    def closure(): #closure function must be defined for pytorch
                        ...
                        # NOTE:
                        # I needed to use this line instead of the line below for some reason
        ...
        # NOTE:
        # (5) Release lock to allow Allocator to execute startTask
        state['decider_pending'] = True
        state['taskNumber'] = -1
        state['decider_cv'].notify_all()
        state['decider_lock'].release()
    ...
    #res_mat_file_name = "results/atta_eta/atta_caseII_etacomparison_eta5_" + str(iter) + ".mat"
